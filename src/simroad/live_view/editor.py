...
class Layout(Model):
    ...
    def references(self):
        ...
        for road in self.roads:
            ...
            if key in pairs:
                raise ValueError(
                    "Duplicate roads between the same junctions; change the existing road instead"
                )
            pairs.add(key)
        for crossing in self.crossings:
            ...
            if crossing.node not in (road.a, road.b):
                raise ValueError(
                    "Crossings must sit at a road endpoint; split the road for a midblock crossing"
                )
        for node in self.nodes:
            kinds = {c.kind for c in self.crossings if c.node == node.id}
            if len(kinds) > 1 or (node.signal and "zebra" in kinds):
                raise ValueError(
                    "A signal-controlled junction needs signalized crossings; move the zebra crossing midblock"
                )
        return self
        ...
        for second in layout.roads[i + 1 :]:
            ...
            if abs(det) < 1e-8:
                if abs((c.x - a.x) * dy - (c.y - a.y) * dx) < 1e-6:
                    ...
                    bounds = sorted(
                        (
                            ((c.x - a.x) * dx + (c.y - a.y) * dy) / norm,
                            ((d.x - a.x) * dx + (d.y - a.y) * dy) / norm,
                        )
                    )
                    if min(1, bounds[1]) - max(0, bounds[0]) > 1e-6:
                        raise ValueError(
                            "Roads overlap. Delete the overlapping segment or join at an endpoint"
                        )
                continue
        ...
        if road and crossing.kind != "informal":
            crossings.append(
                EditorCrossing(
                    ...
                    node=node_ids[crossing.node],
                    road=road,
                    kind=crossing.kind,
                    walk_seconds=crossing.min_walk_seconds,
                )
            )
    zones = []
    for zone in bundle.zones:
        ...
        zones.append(
            EditorZone(
                ...
                x=x,
                y=y,
                radius=min(1000, max(15, zone.radius_m)),
...
def compile_layout(layout, base, directory):
    ...
    for road in layout.roads:
        ...
        for index, identifier in enumerate(road_ids[road.id]):
            ET.SubElement(
                ...
                id=identifier,
                attrib={
                    "from": road.b if index else road.a,
                    "to": road.a if index else road.b,
                    "numLanes": str(road.lanes),
                    "speed": str(road.speed / 3.6),
                    "width": "3.2",
                    "sidewalkWidth": "2",
                },
            )
    for crossing in layout.crossings:
        ET.SubElement(
            ...
            "crossing",
            node=crossing.node,
            edges=" ".join(road_ids[crossing.road]),
            priority="true",
            width="4",
        )
    for root, name in (
        (node_xml, "roads.nod.xml"),
        (edge_xml, "roads.edg.xml"),
        (connections, "roads.con.xml"),
    ):
        write_xml(root, directory / name)
    args = [
        binary("netconvert"),
        "--node-files",
        directory / "roads.nod.xml",
        "--edge-files",
        directory / "roads.edg.xml",
        "--connection-files",
        directory / "roads.con.xml",
        "--walkingareas",
        "true",
    ...
    for c in layout.crossings:
        if not any(
            e.get("function") == "crossing"
            and e.get("id", "").startswith(f":{c.node}_c")
            and set(road_ids[c.road]).issubset(e.get("crossingEdges", "").split())
    ...
    rng = random.Random(0)
    rng.shuffle(edges)
    offsets = list(range(1, len(edges)))
    rng.shuffle(offsets)
    pairs = ((a, edges[(i + offset) % len(edges)]) for offset in offsets for i, a in enumerate(edges))
    selected = []
    for a, b in pairs:
    ...
    files = {
        ...
        "zones.yaml": {"zones": zones},
        "zone_defaults.yaml": defaults,
        "fleet.yaml": base.fleet.model_dump(),
        "infrastructure.yaml": {
            "crossings": [
                {
                    "node": c.node,
                    "edges": road_ids[c.road],
                    "kind": c.kind,
                    "min_walk_seconds": c.walk_seconds,
                }
                for c in layout.crossings
            ],
            "stops": [],
            ...
            "gateways": [
                {
                    "edge": a,
                    "to_edge": b,
                    "vehicles_per_hour": layout.traffic_per_hour / len(selected),
                    "begin_hour": sim["begin"] / 3600,
                    "end_hour": sim["end"] / 3600,
                }
                for a, b in selected
            ],
        },
        "project.yaml": {
            ...
            "city": "city.yaml",
            "zones": "zones.yaml",
            "zone_defaults": "zone_defaults.yaml",
            "fleet": "fleet.yaml",
            "demand": "demand.yaml",
